  ...
  <div class="container mx-auto p-4 w-full pb-16">
    <div x-data="{ isAdd: false }">
      <div class="flex justify-end py-6">
        <div @click="isAdd = !isAdd" class="bg-gray-800 text-white py-2 px-4 text-sm rounded-md shadow-lg hover:bg-gray-700 duration-200">
    ...
    <div x-data="{ open: false }" class="grid py-4 border-gray-700 shadow-xl rounded-lg border px-4 max-w-sm sm:max-w-full mb-4">
      <div class="grid grid-cols-3 py-2">
        ...
        <div class="col-span-1 text-end grid gap-2 sm:flex sm:justify-end items-center sm:gap-2">
          <button @click="open = !open" class="bg-green-600 py-1 px-2 text-white text-sm rounded hover:bg-green-500 duration-200">
            ...
            @foreach ($absen as $absen)
              @if ($absen->user->level !== '3')
               ...
               @if ($absen->status == null)
                <div class="col-start-5 col-end-7 text-red-500 sm:text-center text-start py-2">No Attendance Yet</div>
                @else
                <div class="col-start-5 col-end-7 text-green capitalize sm:text-center text-start py-2">{{ $absen->status }}</div>
               @endif
                <div class="col-start-8 col-end-13 font-semibold text-center py-2">
                  <form class="sm:flex justify-center grid gap-1" action="{{ route('absensi.update', $absen->id) }}" method="post">
                    @csrf
                    @method('PATCH')
